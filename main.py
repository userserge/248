...
if __name__ == "__main__":
    ...
    parser.add_argument('--evaluator_model_path', default='models/target_ckp/FaceNet_95.88.tar',
                        help='path to evaluator_model')
    ...
    parser.add_argument('--experiment_name', type=str, default='default_test1',
                        help='experiment name for experiment directory', required=True)

    parser.add_argument('--config_file', type=str, help='config file that has attack params', required=True)
    parser.add_argument('--private_imgs_path', type=str, default='',
                        help='Path to groundtruth images to copy them to attack dir. Empty string means, our tool will not copy.')
    ...
    ckp_target_model = torch.load(path_target_model, map_location=device)
